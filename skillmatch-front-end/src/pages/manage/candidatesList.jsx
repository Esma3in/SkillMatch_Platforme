...
export default function CandidateList() {
  ...
  const StatusIndicator = ({ status }) => {
    ...
    switch(status) {
      case 'Done':
        return <span className="flex items-center text-green-600"><CheckCircle size={16} className="mr-1" /> Done</span>;
      case 'In Progress':
      case 'Pending':
        return <span className="flex items-center text-yellow-600"><Clock size={16} className="mr-1" /> Pending</span>;
      case 'Failed':
        return <span className="flex items-center text-red-600"><XCircle size={16} className="mr-1" /> Failed</span>;
      case 'No Tests':
  ...
  const acceptCandidate = async (candidateId) => {
    try {
      const response = await api.put(`/candidates/${candidateId}/accept`);
  ...
  const rejectCandidate = async (candidateId) => {
    try {
      const response = await api.put(`/candidates/${candidateId}/reject`);
              ...
              <td className="py-3 px-4">
                <div className="flex items-center">
                  <div className="text-green-600 flex items-center">
                    <span className="mr-1">⬆</span> {candidate.tests && candidate.tests[0] ? candidate.tests[0].status : '0'}
                  </div>
                </div>
              </td>
              <td className="py-3 px-4">
            ...
            <div>
              ...
              <div className="mb-6">
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold">{candidateDetails.candidate?.name || selectedCandidate?.name || 'Unknown'}</h3>
                    <p className="text-gray-500">{candidateDetails.candidate?.email || selectedCandidate?.email || 'No email'}</p>
                  </div>
                </div>
                          ...
                          <tr key={test.id} className="border-b">
                            ...
                            <td className="py-2 px-4">
                              <StatusIndicator status={
                                test.status > 50 ? 'Done' : 
                                test.status > 0 ? 'In Progress' : 'Failed'
                              } />
                            </td>
                          </tr>
                        ))}
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex mb-4 border-b">
        ...
        <button
          className={`px-4 py-2 ${activeTab === 'topRanked' ? 'border-b-2 border-blue-600 text-blue-600 font-medium' : 'text-gray-500'}`}
          ...
          {(activeTab === 'all' && sortedCandidates.length > itemsPerPage) || 
           (activeTab === 'topRanked' && topRankedCandidates.length > itemsPerPage) ? (
            <div className="flex justify-between items-center mt-4">
              ...
              <span>Page {currentPage} of {Math.ceil((activeTab === 'all' ? 
                sortedCandidates.length : topRankedCandidates.length) / itemsPerPage)}</span>
              <button
                onClick={() => setCurrentPage(prev => {
                  const maxPage = Math.ceil((activeTab === 'all' ? sortedCandidates.length : topRankedCandidates.length) / itemsPerPage);
                  return prev < maxPage ? prev + 1 : prev;
                })}
                disabled={currentPage >= Math.ceil((activeTab === 'all' ? 
                  sortedCandidates.length : topRankedCandidates.length) / itemsPerPage)}
                className="px-3 py-1 bg-gray-300 rounded disabled:opacity-50 hover:bg-gray-400 transition-colors"
              >
                <ArrowRight size={16} />
              </button>
