                ...
                @foreach($problems as $problem)
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        <td class="py-3 px-6 text-left whitespace-nowrap">
                            {{ Str::limit($problem->name, 30) }}
                        </td>
                        <td class="py-3 px-6 text-left">
                            {{ Str::limit($problem->description, 50) }}
                        </td>
                        <td class="py-3 px-6 text-left">
                        ...
                        <td class="py-3 px-6 text-left">
                            ...
                            @endphp
                            <span class="text-xs font-semibold px-2 py-1 rounded {{ $levels[$problem->level] ?? 'bg-gray-200' }}">
                                {{ ucfirst($problem->level) }}
                            </span>
                        </td>
                        <td class="py-3 px-6 text-left">
                            @foreach ($problem->candidates as $candidate)
